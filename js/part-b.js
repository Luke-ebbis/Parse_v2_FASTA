    ...
    displaySummary(proteins) {
        ...
        proteins.forEach((x, i) => {
            ...
            data.push({
                ...
                p_sum: x.residues.filter(x => x.region == 'P').reduce((a, c) => a + c.dist_norm, 0).toFixed(3),
                longest_ps_idr: longest.P.length,
                p_start: longest.P.start,
                p_end: longest.P.end,
                d_sum: x.residues.filter(x => x.region == 'D').reduce((a, c) => a + c.dist_norm, 0).toFixed(3),
                longest_nonPS_IDR: longest.D.length,
                d_start: longest.D.start,
                d_end: longest.D.end,
                f_sum: x.residues.filter(x => x.region == 'F').reduce((a, c) => a + c.dist_norm, 0).toFixed(3),
